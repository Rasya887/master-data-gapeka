      ...
      <div class="data-card">
        <div class="card-header">
          <div class="header-info">
            ...
            <div>
              <h2 class="header-title">Tabel Data Menu</h2>
              <p class="header-subtitle">Kelola menu sistem</p>
            </div>
          </div>
          <Link href="/menus/create" class="btn-primary">
            + Tambah Menu
          </Link>
        </div>
        ...
        <div class="form-container">
          <div class="table-container">
            <table class="w-full text-sm text-left">
              ...
              <tbody>
                <tr v-for="(menu, index) in menus" :key="menu.id" class="hover:bg-gray-50">
                  <td class="p-2 border">{{ index + 1 }}</td>
                  <td class="p-2 border truncate">{{ menu.name || '-' }}</td>
                  <td class="p-2 border truncate">{{ menu.route || '-' }}</td>
                  <td class="p-2 border truncate">{{ menu.menu_order || '-' }}</td>
                  <td class="p-2 border">
                    <span
                      :class="[
                  ...
                  <td class="p-2 border space-x-2">
                    <Link :href="`/menus/${menu.id}/edit`" class="btn-primary btn-sm bg-yellow-600 hover:bg-yellow-700">Edit</Link>
                    <button @click="hapus(menu.id)" class="btn-danger-sm">Hapus</button>
                  </td>
                </tr>
              </tbody>
            </table>

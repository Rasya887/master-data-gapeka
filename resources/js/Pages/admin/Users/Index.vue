      ...
      <div class="data-card">
        <div class="card-header">
          <div class="header-info">
            ...
            <div>
              <h2 class="header-title">Tabel Data User</h2>
              <p class="header-subtitle">Kelola data pengguna sistem</p>
            </div>
          </div>
            ...
            <table class="w-full text-sm text-left">
              ...
              <tbody>
                <tr v-for="(user, index) in users" :key="user.id" class="hover:bg-gray-50">
                  ...
                  <td class="p-2 border truncate">{{ user.name || '-' }}</td>
                  <td class="p-2 border truncate">{{ user.email || '-' }}</td>
                  <td class="p-2 border truncate">{{ user.roles[0]?.name ?? '-' }}</td>
                  <td class="p-2 border space-x-2">
                    <Link :href="`/users/${user.id}/edit`" class="btn-primary btn-sm bg-yellow-600 hover:bg-yellow-700">Edit</Link>
                    <button @click="hapus(user.id)" class="btn-danger-sm">Hapus</button>
                  </td>
                </tr>
              </tbody>
            </table>
